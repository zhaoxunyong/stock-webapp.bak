...
{
	//JS正则验证：money是至少1位的整数或浮点数
	if(money==null||false==/^-?\d+(\.\d*)?$/.test(money)){
		return "";//空字符串、无效字符串
	}
	if(money==null||isNaN(parseFloat(money))){
		return "";//空字符串、无效字符串
	}
	var symbol='';
...
(function($, undefined) {
    ...
    /*
     * showType:定义消息窗体如何顯示 可用值有: null,slide,fade,show. 默认值是slide.
     * showSpeed: 定义消息窗体最终顯示的毫秒数. 默认值是 600.
     * width: 定义消息窗体宽度.默认值是 250.
     * height: 定义消息窗体高度. 默认值是100.
     * msg: 顯示的消息文本.
     * title: 标题文本顯示到panel的头部的.
     * timeout: 如果定义为 0,消息窗体将不会關閉除非用户關閉.定义为不是0,消息窗体将在時間超时后自动關閉. 
     */
    ...
    /*
     * 顯示一个alter窗体.参数:
     * title: 标题文本,顯示在panel的头部的.
     * msg: 顯示的消息文本.
     * icon:warning顯示icon图片,可用值有: error,question,info,warning.
     * fn: 窗体關閉的时候触发的回调函数. 
     */
    ...
    //};
    /*
     * 顯示一个確認消息窗体有一个OK和一个Cancel按钮,参数:
     * title:标题文本顯示在panel的头部的.
     * msg:顯示的消息文本.
    ...
    //};
    /*
     * 顯示一个消息窗体,一个OK和一个Cancel按钮,提示用户輸入一些文本,参数:
     * title: 顯示到panel头部的标题文本.
     * msg: 顯示的消息文本.
     * fn(val): 回调函数,value参数是用户輸入的值. 
     */
     ...
     * 顯示一个进度消息窗体. 
     * 选项定义如下:
     * title: 顯示到panel头部的标题文本,默认值''. 
     * msg: 消息框的body文本,默认值''. 
     * text: 这个文本顯示到进度条上, 默认未定义. 
	...
	if($.validator){
		$.validator.config({
		    ...
		    rules: {
	            ...
	            ,qq: [/^[1-9]\d{4,}$/, "QQ号格式不正确"]
	            ,date: [/^\d{4}-\d{1,2}-\d{1,2}$/, "請輸入正确的日期,例:yyyy-mm-dd"]
	            ,time: [/^([01]\d|2[0-3])(:[0-5]\d){1,2}$/, "請輸入正确的時間,例:14:30或14:30:00"]
	            ,ID_card: [/^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])((\d{4})|\d{3}[A-Z])$/, "請輸入正确的身份證号码"]
	            ,url: [/^(https?|ftp):\/\/[^\s]+$/i, "网址格式不正确"]
	            ,postcode: [/^[1-9]\d{5}$/, "邮政编码格式不正确"]
	            ,chinese: [/^[\u0391-\uFFE5]+$/, "請輸入中文"]
	            ,username: [/^\w{3,12}$/, "請輸入3-12位数字、字母、下划线"]
	            ,password: [/^[0-9a-zA-Z]{6,16}$/, "密碼由6-16位数字、字母组成"]
...
//将指定form的所有input清空，一般用于清空查詢条件，传入表单ID
